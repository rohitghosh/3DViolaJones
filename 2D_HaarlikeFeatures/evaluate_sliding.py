...
def evaluate_pic(classifier,path, threshold, stride_size,window_length):
    stride = (stride_size, stride_size)
    window_size = (window_length, window_length)#heuristically chosen for this image
    ...
    results = [+1 if i > threshold else -1 for i in results_lists ]
    print (len(results))
    print (sum(results))
    #print (results_lists)
